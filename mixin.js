...
const minxin = {
	...
	computed: {
		...
		safeAreaHeight() {
			return this.isIphoneX && this.safeAreaInsetBottom
	...
	methods: {
		...
		getTeach() {
			...
			if (isTeach == null || isTeach == "") {
				...
				const isShare =
					this.$mp.page.route == "pages/class/class-detail/class-detail";
		...
		getLocation(isShow = true) {
			...
			return new Promise((resolve, reject) => {
				uni.getLocation({
					type: "gcj02",
					success: function (res) {
						console.log(self.isLocation + "当前位置的经度：" + res.longitude);
						...
						uni.getSetting({
							success: (res) => {
								if (
									...
									if (self.location && self.location.latitude) {
		...
		lower: throttle(function (e) {
			if (
				this.queryParams.page * this.queryParams.row >=
