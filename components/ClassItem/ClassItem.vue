	...
	<view class="class-content" @click="handleNavTo">
		...
		<view class="class-content-bottom">
			<view class="class-content-left">
				<van-image use-error-slot class="class-content-img" fit="cover" radius="10" width="140" height="132"
					:src="data.coverImage">
				</van-image>
			</view>
			<view class="">
				...
				<view class="class-content-cycle" v-if="data.startPeriod || data.startTime">
					...
					<text>上课时段：
						<text class="o-over over-time">
							{{data.courseType==2?(data.CourseDateName||''):(data.weekCodeName||'')}}
						</text>
						{{data.startPeriod || data.startTime || ''}}~{{data.endPeriod || data.endTime ||''}}</text>
				</view>
				<!-- 教练 -->
