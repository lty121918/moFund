<template>
	<view class="pin">
		<view class="pin-content" @click="$utils.router.navTo($page.OrderInfo,{classId:item.classInfoId})">
			<view class="pin-content-top">
				<image class="pin-content-img" :src="item.headUrl" mode="aspectFill"></image>
				<view class="pin-content-center">
					<view>{{item.nickName || '微信昵称'}}</view>
					<!-- 上课周期 -->
					<view class="pin-content-cycle" v-if="item.startDate">
						<image class="pin-content-cycle-img" src="/static/class/cycle.png" mode="widthFix"></image>
						<text>上课周期：{{item.startDate}}~{{item.endDate}}</text>
					</view>
					<!-- 上课时段 -->
					<view class="pin-content-cycle" v-if="item.startPeriod">
						<image class="pin-content-cycle-img" src="/static/class/time.png" mode="widthFix"></image>
						<text>上课时段：
						<text class="o-over over-time">{{item.courseType==2?(item.CourseDateName||''):(item.weekCodeName||'')}} </text>
						{{item.startPeriod || ''}}~{{item.endPeriod||''}}</text>
					</view>
