    ...
    <view class="class-detail-user" v-if="data.phone">
      ...
      <view class="class-detail-head" v-if="data.classAddressBookCoachVO">
        <view class="class-detail-head-flex">
          <image
            class="class-detail-head-img"
            :src="data.classAddressBookCoachVO.avatar"
            mode="aspectFill"
          ></image>
          <text class="fwb fz32">{{
            data.classAddressBookCoachVO.nickName || "微信昵称"
          }}</text>
          <image
            class="class-detail-head-icon"
            src="/static/class/head.png"
        ...
        <view
          class="class-detail-head-contact"
          :data-phone="data.classAddressBookCoachVO.phone"
          @click="handlePhone"
        >
      ...
      <view class="class-detail-stu">
        <view
          ...
          v-for="(row, index) in classAddressBookStudentList"
          :key="row.id"
        >
          <image
          ...
          <view
            class="class-detail-stu-contact"
      ...
      this.$http["classes"]
        ...
        .then((res) => {
          if (res.code == 200) {
            ...
            res.data.classAddressBookCoachVO.avatar = remoteAvatar(
              res.data.classAddressBookCoachVO.avatar
            );
            for (
              let i = 0;
              i < res.data.classAddressBookStudentList.length;
              i++
            ) {
              res.data.classAddressBookStudentList[i].avatar = remoteAvatar(
                res.data.classAddressBookStudentList[i].avatar
              );
            }
            this.data = res.data;
            this.classAddressBookStudentList = JSON.parse(
              JSON.stringify(res.data.classAddressBookStudentList)
            );
