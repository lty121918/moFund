  ...
  <view class="course">
    ...
    <view class="course-nearby">
      <view
        ...
        @click="$utils.router.navTo($page.Search, { type: 'no', productId })"
      >
        <image
      ...
      <view
        class="course-nearby-content"
        v-for="item in productSpellClassList"
        :key="item"
      >
        <image
          class="course-nearby-content-img"
          :src="item.headUrl"
          mode="aspectFill"
        ></image>
        <view>
          <view class="course-nearby-content-center">
            <text class="course-nearby-content-num">
              最多{{ item.maxNum }}人
            </text>
          ...
          <view class="course-nearby-content-cycle">
            <image
              class="course-nearby-content-cycle-img"
        ...
        <view v-if="item.useStudentNum < item.maxNum">
          <view
            class="course-nearby-content-button"
            :class="[productInfo.sell ? '' : 'dis-color']"
            @click="handleNavTo(item)"
          >
            加入拼班
          </view>
          <view class="course-nearby-content-success" v-if="item.minNumMax">
            差{{ item.minNumMax }}人拼成
          </view>
          <view class="course-nearby-content-success" v-else>
            差{{ item.unUseNum }}人拼满
          </view>
        </view>
        <view v-else>
    ...
    <view class="course-detail">
      ...
      <view class="course-detail-tab">
        <view
          class="course-detail-tab-item"
          @click="handleTab(1)"
          :class="[active == 1 ? 'course-detail-tab-active' : '']"
        >
          课程详情
        </view>
        <view
          class="course-detail-tab-item"
          @click="handleTab(2)"
          :class="[active == 2 ? 'course-detail-tab-active' : '']"
        >
          用户评价
        </view>
      </view>
      ...
      <view class="" v-if="active == 2">
        <view
          class="course-detail-evaluate"
          v-for="item in productEvaluate"
          :key="item.id"
        >
          <image
            class="course-detail-evaluate-img"
            :src="item.avatar"
            mode="aspectFill"
          ></image>
          <view>
            <view class="course-detail-evaluate-info">
              <view class="course-detail-evaluate-info2">
                ...
                <view class="course-detail-evaluate-info3">
                  <uni-rate
                    v-model="item.evaluationLevel"
                    disabled
                    disabledColor="#DE501F"
                    size="20"
                    color="#838899"
                    active-color="#DE501F"
                  />
        ...
        <view
          v-if="isEvaluate"
          class="course-detail-evaluate-more"
          @click="$utils.router.navTo($page.Evaluate, { productId })"
        >
          <text>查看更多</text>
          <image
            class="course-detail-evaluate-more-img"
    ...
    <!-- 我要拼班 -->
    <view
      class="course-footer2"
      :style="{ marginBottom: `${safeAreaHeight}px` }"
    ></view>
    <view class="course-footer">
      <view
        class="course-footer-button"
        :class="[productInfo.sell ? '' : 'dis-color']"
        :style="{ marginBottom: `${safeAreaHeight}px` }"
        @click="submit"
      >
...
export default {
      ...
      if ((e.lat && e.lng) || e.campusId) {
        ...
        if (res.code == 200) {
          ...
          const ls =
            list.filter(
              item =>
                item.campusId == e.campusId ||
                (item.lat == e.lat && item.lng == e.lng)
            )[0] || null
          console.log('当前校区', ls)
  ...
  onShareAppMessage(res) {
    ...
    return {
      ...
      path: `${this.$page.CourseDetail}?productId=${this.productId}&lat=${
        this.campusOther.lat
      }&lng=${this.campusOther.lng}&campusId=${this.campusOther.campusId}`
    }
  },
  onShareTimeline(res) {
    //分享到朋友圈
  ...
  methods: {
    ...
    async getCourseDetail() {
      ...
      if (res.code == 200) {
        uni.setNavigationBarTitle({ title: res.data.productName })
      ...
      // 获取商品详情下的描述
      const res2 = await getProductDetails({ productId: this.productId })
      if (res2.code == 200) {
        res2.data = res2.data.replace(
          /<img /g,
      ...
      // 获取当前商品的评价
      const res3 = await getProductEvaluate({ productId: this.productId })
      if (res3.code == 200) {
