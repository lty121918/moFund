  ...
  <view class="order">
    <view class="order-tab">
      <view
        ...
        @click="handleTab(1)"
        :class="[active == 1 ? 'order-tab-active' : '']"
      >
        全部订单
      </view>
      <view
        class="order-tab-item"
        @click="handleTab(2)"
        :class="[active == 2 ? 'order-tab-active' : '']"
      >
        消费订单
      </view>
      <view
        class="order-tab-item"
        @click="handleTab(3)"
        :class="[active == 3 ? 'order-tab-active' : '']"
      >
        充值订单
      </view>
    </view>
  ...
  methods: {
    ...
    async search() {
      ...
      return new Promise(async (resolve, reject) => {
        let data = []
        const { getOrder, getOrderConsume, getOrderInvest } = self.$http['mine']
        let res
