  ...
  <view class="login">
    <view class="login-content">
      <image
        ...
        src="/static/login/logo2.png"
        mode="widthFix"
      ></image>
      <view class="fz40">
        {{ CONFIG.DEFAULT_SYSTEM_TITLE }}申请获得以下权限
      </view>
      <view class="color666 mt32">(获得您的手机号)</view>
      <button
        v-if="isSubmit || protocolData.length == 0"
        class="login-button"
        type="primary"
      ...
      <view
        class="tip-box flex"
        v-if="protocolData.length > 0"
        @click="changeBox"
      >
        <view v-if="!isSubmit" class="attendance-check"></view>
        <image
          v-if="isSubmit"
        ...
        ></image>
        <view>登录即视为您同意</view>
        <view class="blue" @click.stop="showModal(protocolData[0], 1)">
          《{{ protocolData[0].protocolTitle }}》
        </view>
        <view v-if="protocolData[1]">及</view>
        <view
          class="blue"
          @click.stop="showModal(protocolData[1], 2)"
          v-if="protocolData[1]"
        >
          《{{ protocolData[1].protocolTitle }}》
        </view>
      </view>
    </view>
    ...
    getphonenumber2: debounce(function(e) {
      ...
      if (!self.isSubmit) {
        ...
        self.$utils.model.showToast(
          `需您阅读《${protocolTitle}》、《${protocolTitle2}》。`
        )
        return false
      }
    }),
