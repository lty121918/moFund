<template>
	<view class="class">
		...
		<view
			class="default-empty"
			v-if="
				(isTeach == 2 && data.length === 0) ||
				(isTeach == 1 && dataList.length === 0)
			">
			<image
		...
		<view
			v-if="officialAccount && isTeach == 1"
			:style="{
	...
	methods: {
		...
		search() {
			...
			return new Promise(async (resolve, reject) => {
				...
				console.log("class请求");
				const { getClassStudentPage, getClassList } = self.$http["classes"];
					...
					if (res.code == 200) {
						...
						if (isTeach == 1) {
							...
							data.forEach((item) => {
								...
								const wait = item.classWxPageVOList.filter(
									(ls) => ls.classStatus == 1
								);
								const conduct = item.classWxPageVOList.filter(
									(ls) => ls.classStatus == 3
								);
