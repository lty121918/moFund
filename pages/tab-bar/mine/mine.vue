<template>
	<view class="mine">
		<view class="mine-head">
			<view class="mine-head-flex">
				...
				<view>
					<view class="flex-sc">
						<text
							class="mine-head-name"
							v-show="!showName && authorization"
							@click="(showName = true), (name = '')">
						...
						<text class="mine-head-role mine-head-role2" v-if="isTeach == 1"
							>教练</text
						>
					</view>
					<view class="mine-head-balance">
						<image
							class="mine-head-balance-img"
							src="/static/mine/balance.png"
							mode="aspectFill"></image>
						<text @click="setCopy"
							>账户余额：￥{{ userInfo.remainingSum || 0 }}</text
						>
					</view>
				</view>
			</view>
			<view class="mine-head-switch" @click="handleRole">
				<image
					class="mine-head-switch-img"
					src="/static/mine/switch.png"
					mode="aspectFill"></image>
				<text>切换身份</text>
			</view>
			<view class="mine-nav" v-if="isTeach == 2">
				...
				<view class="mine-nav-item" @click="handleRouterTo($page.MyClass)">
					<image
						class="mine-nav-img2"
						src="/static/mine/competition.png"
						mode="aspectFill"></image>
					<view>我的拼班</view>
				</view>
			...
			<view class="mine-operation mine-operation2" v-if="isTeach == 1">
				<view
					class="mine-operation-item mine-operation-item2"
					@click="handleRouterTo($page.Wallet)">
					<view class="mine-operation-item-left">
						<image
							class="mine-operation-item-img"
							src="/static/mine/balance2.png"
							mode="aspectFill"></image>
						<text>我的钱包</text>
					</view>
		...
		<view class="mine-operation" v-if="isTeach == 2">
			<view class="mine-operation-item" @click="handleRouterTo($page.MyCoupon)">
				<view class="mine-operation-item-left">
					<image
						class="mine-operation-item-img"
						src="/static/mine/declare.png"
						mode="aspectFill"></image>
					<text>我的优惠券</text>
				</view>
				<uni-icons type="right" color="#141D3D" size="24" />
		...
		<view v-if="isTeach == 2" class="mine-operation">
			<view
				class="mine-operation-item mine-operation-item2"
				@click="handleRouterTo($page.Coach)">
				<view class="mine-operation-item-left">
					<image
						class="mine-operation-item-img"
						src="/static/mine/apply.png"
						mode="aspectFill"></image>
					<text>成为教练</text>
				</view>
				<uni-icons type="right" color="#141D3D" size="24" />
